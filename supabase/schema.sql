-- Supabase Schema for DevTrack AI Developer Growth Dashboard
...
-- Row Level Security (RLS) Policy suggestions:
-- Enable RLS for all tables.
-- For example:
-- alter table users enable row level security;
-- create policy "Users can view and edit their own profiles." on users for all using (auth.uid() = id);
